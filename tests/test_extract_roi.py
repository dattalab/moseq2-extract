...
def test_plane_ransac():
    ...
    def plane_equation_noisy(xy,noise_scale=0):
        return 2*xy[:,0]+5*xy[:,1]+1+np.random.normal(0,noise_scale,size=(xy.shape[0],))
    ...
    a=plane_ransac(tmp_img,depth_range=(0,1000),iters=1000,noise_tolerance=10)
    norma=-a[0]/a[0][2]

    npt.assert_almost_equal(norma[[0,1]],np.array([2,5]),2)
